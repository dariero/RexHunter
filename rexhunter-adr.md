...
> **Reconciliation note (logged in the frontend spine): the runs ⊕ overlay tier.** The frontend assembler (`viewstate.build_viewstate`) overlays each run's `territory`/`outcome` — and a per-territory tier (the latest run per territory) — from the `runs` table onto the pure trajectory projection, exactly as the pen tier overlays verdict status from `pen_events`. This is invariant 2's "tables transactionally maintained alongside it" clause applied, **not** a new event: terminal decisions still emit no trajectory event (the Pillar 2/4 reconciliation notes stand), and the pure reducer (`view.py`) never reads a table — the overlay lives in the assembler only. One deferral, pinned by test (`test_ghost_cursor_stamps_current_outcome_is_a_documented_deferral`): the ghost (per-run `seq`) cursor stamps the run's *current* outcome, not the outcome as of the scrubbed position — revisit when ghost replay gets a UI. Extended (4a): each run's budget ceilings (`cost_ceiling_usd`, `max_iterations`) are recorded on its `runs` row at start — write-once input facts in the invariant-6 spirit (snapshot now, reconstruct never: a ghost replays under the caps it actually ran under) — and served through this same overlay. Extended (4b): the daemon spend ceiling and the schedule's territory list enter the projection as INJECTED `finalize` inputs — the clock's category under invariant 5 (explicit, never-stored config; not folded, not persisted) — and the territory tier becomes schedule ∪ runs-seen, with a dormant tile (`latest_outcome=None`, `last_started_at=None`) for every scheduled-but-never-hunted territory.
